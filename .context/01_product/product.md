...
Brebaje is a **zero-knowledge proof ceremony management platform**: a Phase 2 Trusted Setup ceremony coordination system for Groth16/zkSNARK circuits. It manages trusted setup ceremonies for zkSNARK circuits—user authentication, ceremony coordination, and participant contributions—with queueing, cryptographic verification, and beacon finalization.
...
- **Users:** General user logic; authenticated via GitHub, Ethereum, or Cardano.
- **Projects:** Container for ceremonies; managed by coordinators.
- **Ceremonies:** Time-bounded events with phases (SCHEDULED → OPENED → CLOSED → FINALIZED; see also PAUSED in protocol).
- **Circuits:** Individual zkSNARK circuits requiring trusted setup; each has its own contribution chain.
...
For the GitHub OAuth flow: (1) Frontend requests GitHub client ID from backend; (2) user completes GitHub OAuth (e.g. device flow or authorization code flow); (3) backend exchanges token for user info and creates/finds user; (4) JWT token is issued for session management.
