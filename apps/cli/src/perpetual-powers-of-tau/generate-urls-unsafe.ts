...
export async function generateUrlsUnsafePerpetualPowersOfTau(
  ...
  try {
    ...
    console.log(`🔗 Generating URL pair for ceremony coordination (UNSAFE MODE)`);
    console.log(`⚠️  WARNING: This command does not verify if files exist in S3`);
    ...
    // UNSAFE MODE: Skip file existence check
    console.log(`⚠️  SKIPPING file existence check for: ${downloadFilename}`);
    ...
    console.log(`⚠️  REMINDER: Download URL will fail if the file doesn't exist in S3!`);
  } catch (error) {
